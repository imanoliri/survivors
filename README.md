...
Keyboard controls remain contextual: `S`, `B`, and `P` focus the direct Scavenge, Build structure, and Produce controls, while `E` requests End Turn. Arrows move the selected map cell while a target panel is open; focused numeric fields use Up/Down; selects use Left/Right with wrapping; Enter invokes the legal primary action; Escape closes an action panel and restores its opener. Active combat cannot be dismissed with Escape.
