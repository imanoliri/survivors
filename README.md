...
The world event is once-per-round setup. Each refuge then automatically reveals its player card for acknowledgement. Scavenge, building and field activities, and Produce can be completed in any order. As a deliberate rewrite UI choice, a compact two-tier activity shelf below the map exposes Scavenge, Produce, Build, Gather, Search survivors, and Form party directly, with their amounts, output, or structure choices alongside the action. At mobile widths up to 600px, each tier becomes a compact horizontally scrollable single row so the controls remain usable without taking over the map vertically. End Turn presents only the calculated consumption. The compact HUD directly above the map shows the current team, resources, activity use, and workers split among wounded, scavenging, idle, used-this-turn, and persistent party assignments.
...
Keyboard controls remain contextual: `S`, `B`, `P`, `G`, and `F` focus the direct Scavenge, Build structure, Produce, Gather amount, and Form Party amount controls, while `E` requests End Turn. `G` and `F` only transfer focus; they do not perform the activity. Arrows move the selected map cell while a target panel is open; focused numeric fields use Up/Down; selects use Left/Right with wrapping; Enter invokes the legal primary action; Escape closes an action panel and restores its opener. Active combat cannot be dismissed with Escape.
